...
class ChiProcessor(DataProcessor):
  ...
  def get_train_examples(self, data_dir):
    """See base class."""
    return self._create_examples(self._read_tsv(os.path.join(data_dir, "news.train")), "train")

  def get_test_examples(self, data_dir):
    """See base class."""
    return self._create_examples(self._read_tsv(os.path.join(data_dir, "news.test")), "test")

  def get_dev_examples(self, data_dir):
    """See base class."""
    return self._create_examples(self._read_tsv(os.path.join(data_dir, "news.dev")), "dev")
...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids,
  ...
  word_size = input_ids.shape[-1].value 
  ...
  with tf.variable_scope("loss"):
    ...
    weight_layer = tf.layers.dense(output_layer, 1, activation=None, kernel_initializer=tf.truncated_normal_initializer(stddev=0.02)) 
    weight_layer = tf.squeeze(weight_layer, axis=2)
    attention_score = tf.nn.softmax(weight_layer, axis=-1)
    ...
    return (loss, per_example_loss, logits, probabilities, attention_score)
...
def model_fn_builder(bert_config, num_labels, init_checkpoint, learning_rate,
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    (total_loss, per_example_loss, logits, probabilities, weights) = create_model(
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
      ...
      output_spec = tf.contrib.tpu.TPUEstimatorSpec(
          mode=mode,
          predictions={"probabilities": probabilities, "weights": weights},
          scaffold_fn=scaffold_fn)
    return output_spec
    ...
    with tf.gfile.GFile(output_predict_file, "w") as writer:
      ...
      for (i, prediction) in enumerate(result):
        ...
        if i >= num_actual_predict_examples:
          break
        output_line = "\t".join(str(class_probability) for class_probability in probabilities) + "|"  +"\t".join(str(word_weight) for word_weight in weights) + "\n" 
